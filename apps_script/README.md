...
보통 1분 안에 시작되고, GitHub 쪽 스크립트(`--send-at 0851`)가 **08:51:00까지 대기했다가**
그 순간 예상시가를 전송 → **매일 08:51** 도착. (08:51 인 이유: 예상체결가 공표 개시가 **08:50:00** 이라서입니다 — 2026-08-20·21 이틀 실측)
...
- 단, 지금이 예상체결가 공표 구간(08:50~09:00) 밖이면 GitHub 스크립트는 "창 종료 후 폴백 1회" 또는
...
| 깨어난 직후 | GitHub 스크립트가 08:51:00까지 대기 |
| **08:51:00** | antc_cnpr 예상시가 수집 → **텔레그램 전송** + 그날 마킹 |
| 이후/중복 | 마커로 즉시 스킵(하루 1건) |

## 백업
- GitHub 워크플로에는 가벼운 예약 백업(08:57·09:05 KST)이 남아 있어, **GAS 가 실패해도** 그날 메시지는 옵니다(폴백).
