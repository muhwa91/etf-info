...
침묵(정상): 주실행 성공 + 발송됨 + 시각 정상. **antc 미확보로 인한 08:53 발송은 설계된
...
MARK_DEADLINE = "⏳ 폴링 데드라인(08:53) 도달"  # antc 미확보(참고 정보)
...
# 발송 지연 임계(KST HHMM). 08:57 이상이면 이상.
#   근거: 설계상 정상 최대치가 08:55 다 — 발송 목표 **08:51**(공표 개시 08:50 + 1분)
#   → antc 폴링 데드라인 **08:53** → KIS 토큰 재시도 마감 `deadline_hm=855`.
#   🔴 **856 이 아니라 857 인 이유 — 토큰 마감 뒤에 «전송 재시도»가 더 붙는다 (2026-08-21 점검).**
#   08:54:5x 에 토큰을 포기하면 네이버 폴백 조회 + `send_telegram_message` 3회 재시도
#   (15s 타임아웃 ×3 + 2s + 4s ≈ 51초)가 이어져 **정상 폴백이 08:56 에 나갈 수 있다.**
#   856 으로 두면 그 정상 경로에 대고 «전송창 넘김» 을 울린다 — 오탐 여유가 9초뿐이었다.
#   그 뒤 발송은 주실행이 제 몫을 못 한 것이므로 이상으로 본다(백업 cron 08:57 발송도 여기 걸린다 —
#   백업이 떴다는 것 자체가 주실행 실패라 종전 설계와 같은 의미다).
#   08:51~08:56 은 antc 미확보 시 설계된 폴백 구간이라 정상으로 둔다.
#   ⚠️ 불변식: 발송목표(851) < 폴링데드라인(853) < 토큰마감(855) < LATE_HM(857) ≤ 백업cron(857).
#   (2026-08-21 — 공표 개시를 **08:50:00 으로 실측 확정**하고 세운 값이다. 종전 830/838/844/845
#    도, 그다음 840/844/846/847 도 전부 개시 시각보다 일러 antc 가 33거래일 내내 0 이었다.
#    🔴 이 숫자들을 조정할 일이 생기면 «개시 08:50» 부터 다시 재라 — 두 번 다 그걸 안 재서 났다.)
LATE_HM = 857
...
def hm(hms):
    """'08:53:22' → 853. 빈 값/이상값은 0(임계 비교에서 정상으로 떨어진다)."""
    try:
...
def diagnose(infos, now_hm=None):
    ...
    """
    if not infos:
        # 전송창(08:56)이 닫히기 전에는 "실행 0건"이 정상이다. cron 은 09:30 이라 정기 실행은
    ...
    if late:
        out.append(("이상", f"발송 : {late[0]['scan']['sent_at'][:5]} (전송창 08:56 넘김)"))
    return out
...
def antc_lines(infos):
    ...
    `발송 : HH:MM (전송창 08:56 넘김)` 항목이 이미 말한다(두 곳에서 내면 중복된다).
    """
    for i in infos:
        s = i.get("scan") or {}
        out = []
        if s.get("source"):
            ...
            if s.get("polls"):
                when = "08:53까지 " if s.get("deadline") else ""
...
# 작업 레포의 알림 기준 문서 맨 위(「한 알림은 한 곳으로」). ※ 이 파일은 공개 미러로 나가므로
# 내부 경로를 적지 않는다.
...
def selftest():
    ...
    #    시각만 새 설계값(폴링 데드라인 08:53)으로 옮겼다 — 마커는 MARK_DEADLINE 과 글자 단위로
    #    일치해야 하고, 감시는 «오늘» 로그만 읽으므로 옛 형식을 보존할 이유가 없다.
    normal = "\n".join(
        [
            ln("23:53:22", "⏳ 예상체결가 대기 폴링... (08:53:08, 시도 12)"),
            ln("23:53:22", "⏳ 폴링 데드라인(08:53) 도달 — antc_cnpr 끝내 미확보, 폴백으로 진행."),
            ln("23:53:22", "💬 텔레그램 메시지 전송 중..."),
            ln("23:53:22", "  ✅ 텔레그램 메시지 전송 성공!"),
            ln("23:53:22", "  🧾 정확도 로그 append: 2026-08-11 · fallback_model · 예상 8,485원"),
        ]
    )
    s = scan(normal)
    assert s["sent_at"] == "08:53:22" and s["polls"] == 12 and s["deadline"], s
    ...
    assert "백업 예약" in texts and "전송창 08:56 넘김" in texts and "주실행 실패" in texts, texts
    ...
    assert "중복 발송 2건" in texts and "전송창 08:56 넘김" in texts, texts

    # ⑤ 임계 경계: 08:56 정상 / 08:57 이상 (LATE_HM=857 — 토큰마감 855 + 전송 재시도 51초).
    assert diagnose([info(1, "workflow_dispatch", "08:24", dict(s, sent_at="08:56:59"))]) == []
    assert diagnose([info(1, "workflow_dispatch", "08:24", dict(s, sent_at="08:57:00"))]), (
        "08:57 은 이상"
    )
    ...
    # 아침 전 가드 — 전송창(08:53) 안이면 "실행 0건"은 정상, 닫힌 뒤부터 이상.
    # 경계를 양쪽으로 짚는다: 856 은 아직 전송 가능 시각이고 857 부터가 지연이다.
    ...
    assert "- 발송 : 10:52 (전송창 08:56 넘김)" in body, body
    assert "- 값 : 추정치 — 예상 시가 못 받음 (08:53까지 12번 요청)" in body, body
